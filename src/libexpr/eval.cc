...
EvalState::TecnixEvalData & EvalState::tecnixEvalData()
{
    return *tecnixData;
}

const EvalState::TecnixEvalData & EvalState::tecnixEvalData() const
{
    return *tecnixData;
}
...
{
    Value * v2 = allocValue();
    // Do a raw storage copy since `operator =` barfs on thunks. Do not copy
    // Value-level tracking certificates; constants are installed before any
    // Tecnix dependency-producing evaluation owns their contents.
    memcpy(static_cast<Value::Storage *>(v2), static_cast<Value::Storage *>(&v), sizeof(Value::Storage));
    v2->clearTrackedSourceAccessSet();
...
struct ExprParseFile : Expr
{
    ...
    void eval(EvalState & state, Env & env, Value & v) override
    {
        ...
        auto e = state.settings.pureEval && path.accessor->tracksEvalAccesses(path.path)
                     ? state.parseExprFromFileCached(path)
                     : state.parseExprFromFile(path);
...
void EvalState::evalFile(const SourcePath & path, Value & v, bool mustBeTrivial)
{
    ...
    auto useTrackedImportResolutionCache = trackingCtx && path.accessor->tracksEvalAccesses(path.path);
    auto & activeImportResolutionCache = useTrackedImportResolutionCache ? *tecnixData->trackedImportResolutionCache
                                                                         : *tecnixData->importResolutionCache;

    auto cachedResolvedPath = getConcurrent(activeImportResolutionCache, path);
    std::optional<EvalImportResolutionCacheEntry> resolvedEntry;

    if (cachedResolvedPath) {
        resolvedEntry = *cachedResolvedPath;
        if (useTrackedImportResolutionCache && resolvedEntry->sourceDeps != emptyEvalSourceAccessSetId)
            recordTrackedSourceAccessSetDependency(*trackingCtx, resolvedEntry->sourceDeps);
    } else if (useTrackedImportResolutionCache) {
        TrackedSourceDepsScope sourceDepsScope(*trackingCtx);
        resolvedEntry.emplace(
            EvalImportResolutionCacheEntry{
                .resolvedPath = resolveExprPath(path),
                .sourceDeps = emptyEvalSourceAccessSetId,
            });
        resolvedEntry->sourceDeps = sourceDepsScope.finish();
        activeImportResolutionCache.emplace(path, *resolvedEntry);
    } else {
        resolvedEntry.emplace(EvalImportResolutionCacheEntry{.resolvedPath = resolveExprPath(path)});
        activeImportResolutionCache.emplace(path, *resolvedEntry);
    }

    auto useTrackedFileEvalCache =
        trackingCtx && resolvedEntry->resolvedPath.accessor->tracksEvalAccesses(resolvedEntry->resolvedPath.path);
    auto & activeFileEvalCache = useTrackedFileEvalCache ? *tecnixData->trackedFileEvalCache : *fileEvalCache;
    ...
    activeFileEvalCache.try_emplace_and_cvisit(
...
{
    tecnixData->importResolutionCache->clear();
    tecnixData->trackedImportResolutionCache->clear();
    tecnixData->parsedFileCache->clear();
    // Deliberately NOT cleared: tecnixData->sourceAccessSetGraph. Value labels
    // are graph-local IDs, and values carrying labels survive a file-cache
    // reset (e.g. across a repl reload). Clearing the graph would re-mint IDs
    // and make surviving labels resolve to the wrong paths. The graph is
    // append-only, so retained IDs stay correct forever.
...
StorePath EvalState::copyPathToStore(NixStringContext & context, const SourcePath & path, PosIdx pos)
{
    ...
    // pure cache hit. The source path is still a dependency of the current
    // target, so record it before consulting the cache.
    if (auto trackingCtx = currentTecnixThreadState.trackingContext;
        trackingCtx && path.accessor->tracksEvalAccesses(path.path))
...
Expr * EvalState::parseExprFromFileCached(const SourcePath & path)
{
    path.accessor->recordEvalAccess(path.path);

    auto entry = getConcurrent(*tecnixData->parsedFileCache, path);
    if (!entry) {
        auto newEntry = std::make_shared<ParsedFileCacheEntry>();
        tecnixData->parsedFileCache->emplace(path, newEntry);
        entry = getConcurrent(*tecnixData->parsedFileCache, path).value_or(newEntry);
    }

    std::call_once((*entry)->once, [&]() { (*entry)->expr = parseExprFromFile(path); });
    assert((*entry)->expr);
    return (*entry)->expr;
}
