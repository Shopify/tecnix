...
struct GitRepo
{
    ...
    /**
     * Get list of dirty files by comparing a commit's tree against a workdir.
     * Works with ODB-only repos (no refs needed).
     */
    virtual std::set<CanonPath> getDirtyFilesAgainstTree(const Hash & commitSha, const std::filesystem::path & workdirPath) = 0;
