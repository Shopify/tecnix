...
Opening a blob is just bounds-checking the section offsets, counts, and ranges, then viewing the arrays in place. There is no JSON parse for dependencies, no heap object graph, no pointer patching, and no decoded index to build before lookup can begin. Bulk queries group the requested targets by shard and visit the shards one row at a time, copying each row out of the database and searching its target histories outside the database lock before the next row is read; a hit's payload and dependency output are copied out while the row is loaded, so memory is bounded by one row rather than the whole scope regardless of the history bound. The search is lazy: it fingerprints a source path only when the candidate currently being checked asks about that path, and a per-run memo eliminates repeated fingerprint computations for paths shared between shards, candidates, and targets. A warm hit thus bypasses the entire tracked-evaluation stack, paying only for shard loading, candidate scanning, fingerprint comparison, and output construction.

### 8.1 Cache history and lifecycle

The cache keeps **bounded historical source closures, not per-commit entries.** The target scale is enough recent history to cover ordinary branch switching and merge-queue churn — on the order of 10–32 historical evals — while keeping lookup fast. The bound is the `tecnix-eval-cache-history` setting, whose default of 32 is chosen roughly as the number of distinct source-closure changes a hot target might see in about 24 hours, not as a function of commits per day; a node whose evaluators span more branches can raise it (say to 128) at the cost of larger rows and longer candidate scans. It is a per-key policy applied whenever a shard row is rewritten — the fresh closure is always kept and the oldest beyond the limit are dropped from every history in the row — not a property of the row format, so histories written under one setting remain readable under another, and lowering the setting shrinks existing rows as they are next written. Rows have no size cap of their own; the count bounds are the policy. The only hard ceiling is what SQLite can store in one blob (1 GB in a stock build; the cache reads the connection's actual limit), and a row that would exceed it is discarded with a warning rather than failing the evaluation. For scale, a dense shard costs roughly 0.3 MB per unit of history (measured: ~115 targets × ~290 pair records × 8 bytes). Reuse across commits still comes from re-proving a candidate closure against the current tree, not from trusting the commit that produced it.

Several evaluators on one machine share the database, so an update is a merge, not a replacement: a writer takes the database write lock up front (`begin immediate`), reads the latest committed shard row, inserts its candidates into the decoded histories, and writes the row back in the same transaction, retrying the whole transaction if the database is busy. Concurrent evaluators learning different closures for the same target therefore accumulate candidates for one another rather than overwriting them.
